...
def getCurrentUSDCoinPrice(asset):
...
def getDollarCostAverageWithoutFees(asset, tax, headers, data):
    ...
    if numShares > 0 and total > 0:
        dollarCostAverage = (total / numShares)
        currentAssetPrice = getCurrentUSDCoinPrice(asset)
        ...
        print(f'You purchased {bcolors.WARNING}{numShares}{bcolors.ENDC} ({bcolors.WARNING}${total:.2f} USD{bcolors.ENDC}) units of {asset} for an average of {bcolors.WARNING}${dollarCostAverage:.2f} USD{bcolors.ENDC} per share.\n')
        print(f'{bcolors.OKCYAN}Amount Bought:\t\t{bcolors.ENDC} {numShares}')
        print(f'{bcolors.OKCYAN}Average Price Paid:\t{bcolors.ENDC} {dollarCostAverage:.2f}')
...
def main():
    ...
    if taxPercentage > 1:
        print(f'{bcolors.FAIL}Your tax_percentage must be in decimal form lower than 1. Example: 0.22')
